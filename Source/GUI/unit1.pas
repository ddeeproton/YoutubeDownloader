...
type
  ...
  TForm1 = class(TForm)
    ...
    procedure ExecuteProcessAndWait(cmd: String);
    function getEncoding(): String;
    procedure ConfigSave;
    procedure ConfigLoad;
    procedure WriteFile(Filename, Content : String);
    function ReadFile(Filename: String):String;
    procedure DownloadFile(http, filename: String);
  private
...
procedure TForm1.ExecuteProcessAndWait(cmd: String);
var p: TProcess;
begin
  p := TProcess.Create(nil);
  p.ApplicationName:= '';
...
procedure TForm1.DownloadFile(http, filename: String);
begin
  if FileExists(filename) then DeleteFile(PChar(filename));
  ExecuteProcessAndWait('wget.exe -O "'+filename+'" "'+http+'" --no-check-certificate');
end;
...
begin
  DownloadFile('https://github.com/ddeeproton/YoutubeDownloader/raw/master/lastversion.txt','lastversion.txt');
  ...
  DownloadFile('https://github.com/ddeeproton/YoutubeDownloader/raw/master/Setup%20installation/YoutubeDownloaderSetup_'+lastVersion+'.exe','YoutubeDownloaderSetup_'+lastVersion+'.exe');
